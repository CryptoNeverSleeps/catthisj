...
import org.catthisj.core.listeners.BlockChainListener;
import org.catthisj.store.FlatDB;
import org.catthisj.store.HashStore;
import org.catthisj.store.MasternodeDB;
 ...
 * catthisj for a specific network. You can get an instance of this class through calling {@link #get()}.</p>
 *
...
public class Context {
    ...
    public Context(NetworkParameters params) {
        log.info("Creating catthisj {} context.", VersionMessage.BITCOINJ_VERSION);
     ...
     * independently use catthisj (or possibly alt coin forks of it) can operate correctly.
     *
    ...
    public static Context get() {
        ...
        if (tls == null) {
            if (isStrictMode) {
                log.error("Thread is missing a catthisj context.");
                log.error("You should use Context.propagate() or a ContextPropagatingThreadFactory.");
                throw new IllegalStateException("missing context");
            }
            if (lastConstructed == null)
                throw new IllegalStateException("You must construct a Context object before using catthisj!");
            slot.set(lastConstructed);
            log.error("Performing thread fixup: you are accessing catthisj via a thread that has not had any context set on it.");
    ...
    /**
     * Require that new threads use {@link #propagate(Context)} or {@link org.catthisj.utils.ContextPropagatingThreadFactory},
     ...
     * a {@link org.catthisj.utils.ContextPropagatingThreadFactory}.
     */
    ...
    /**
     * Returns the {@link org.catthisj.core.NetworkParameters} specified when this context was (auto) created. The
    ...
    public void initcatThis(boolean liteMode, boolean allowInstantX) {
    ...
    public void setPeerGroupAndBlockChain(PeerGroup peerGroup, @Nullable AbstractBlockChain chain)
    {
        ...
        if (sporkManager!=null) {
            ...
            log.error("##### catThis init not called!, this is going to be an issue in the future");
        }
        if (instantSend!=null) {
    ...
    BlockChainListener updateHeadListener = new BlockChainListener () {
        ...
        {
            //todo furszy: commented catthis MN stuff
